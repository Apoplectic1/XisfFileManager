...
renames files to a canonical scheme, tracks graded/accepted counts back into the N.I.N.A. Target
Scheduler database, and maintains a calibration-frame library.
...
checksum — the format XFM writes on save.
...
- **Telescopes:** APM107, EvoStar150, Newtonian254 — each optionally with the Riccardi 0.75x
  reducer (focal length and ratio are reducer-aware).
- **Capture software:** NINA, TheSkyX, SGPro, Voyager, SharpCap (`SWCREATE` = NINA, TSX, SGP, VOY, SCP).

## Ecosystem position

XFM is one app in the Astronomy portfolio (see the parent `../CLAUDE.md` map). It reads/writes the
Target Scheduler plugin's `scheduler.db` directly (graded counts). Long-term, the Target Scheduler
tab migrates to the TSM app and XFM consumes `Catalog.db` read-only via `Astronomy.Catalog`
(ROADMAP follow-up #8).
